      ...
      <section className="py-16 bg-slate-50 border-b border-slate-200/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-8 border-b border-slate-200">
            ...
            <Link
              href="/destinations"
              className="inline-flex items-center gap-2 text-xs font-bold text-[#1E90FF] hover:underline"
            >
            ...
            {DESTINATIONS.slice(0, 3).map((dest) => (
              <GlassCard key={dest.id} className="p-0 overflow-hidden border border-slate-200 bg-white shadow-xs group">
                ...
                <div className="p-4 space-y-3">
                  ...
                  <Link href={`/destinations/${dest.id}`} className="block w-full text-center py-2 bg-slate-100 hover:bg-[#EAF4FF] text-slate-800 hover:text-[#1E90FF] font-bold text-xs rounded-xl transition-colors">
            ...
            {PROGRAMS.slice(0, 3).map((prog) => (
              <GlassCard key={prog.id} className="p-6 border border-slate-200 bg-white shadow-xs flex flex-col justify-between">
                ...
                <div className="pt-4 mt-4 border-t border-slate-100">
                  <Link href={`/programs/${prog.id}`} className="inline-flex items-center justify-between w-full text-xs font-bold text-[#1E90FF] hover:underline">
      ...
      <section className="py-12 bg-[#003875] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
          ...
          <div className="pt-2 flex flex-wrap justify-center gap-4">
            <Button variant="primary" size="md" onClick={() => setBookingOpen(true)} className="text-xs font-bold">
